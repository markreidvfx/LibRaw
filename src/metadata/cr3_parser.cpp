...
int LibRaw::selectCRXTrack()
{
  ...
  for (int i = 0; i <= maxTrack && i < LIBRAW_CRXTRACKS_MAXCOUNT; i++)
  {
    ...
    if(d->MediaType > 0)
    {
      if(err = selectCRXFrame(i, frame_select))
        return err;
    }

    if (d->MediaType == 3) // CTMD metadata
    {
      if(err = parseCR3_CTMD(i))
        return err;
    }
...
#define current_track libraw_internal_data.unpacker_data.crx_header[nTrack]
    ...
    else if (!strcmp(AtomNameStack, "moovtrakmdiaminfstblstsz"))
    {
      if (szAtomContent >= 12)
      {
        ...
        if(current_track.MediaType == 1 && entries > imgdata.idata.raw_frame_count)
          imgdata.idata.raw_frame_count = entries;
